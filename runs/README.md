...
- `causal-window/candidate-512pt/<architecture>_<settings-hash>/`: the completed
  lower-memory causal-window run configured by `run_causal.py`.
...
reused. This branch includes the completed 512-point run output; the code
repair itself did not retrain the historical run.
